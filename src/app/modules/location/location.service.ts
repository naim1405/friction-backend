...
import type { ICreateLocationPayload, ILocationFilters } from './location.interface';
...
const getAllLocations = async (
	...
	const exactFilters = Object.entries(filterData).filter(([, value]) => value !== undefined && value !== '').map(([key, value]) => ({
			[key]: { equals: value },
	}));
	...
	const whereConditions: Prisma.LocationWhereInput = andConditions.length ? { AND: andConditions } : {};

	const orderBy = { [sortBy]: sortOrder } as Prisma.LocationOrderByWithRelationInput;
...
const createLocation = async (payload: ICreateLocationPayload) => {
    ...
    const createdLocation = await prisma.location.create({
		data: {
			...
			...(payload.country !== undefined ? { country: payload.country } : {country: 'Bangladesh'}),
